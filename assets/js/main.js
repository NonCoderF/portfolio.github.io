(() => {
  ...
  const suggestedQuestions = ['How did users cheat the squat detector?', 'Tell me about SonicBridge.'];
  $$('.assistant-prompts,.prompt-list').forEach(group => [...group.querySelectorAll('button')].forEach((button, index) => {
  ...
  const resourceTitles = { 'adaptive-exercise-recognition':'Adaptive Exercise Recognition', archguard:'ArchGuard', sonicbridge:'SonicBridge', 'tapori-ai':'Tapori AI', 'biometric-sdk':'Biometric SDK', 'sally-launcher':'Sally Launcher' };
  const inferResources = (resources, prompt, reply) => {
    if (Array.isArray(resources) && resources.length) return resources;
    const text = `${prompt || ''} ${reply || ''}`.toLowerCase();
    return Object.keys(resourceHrefs).map(id => ({ id, title: resourceTitles[id] || id }))
      .filter(resource => text.includes(resource.title.toLowerCase()) || text.includes(resource.id.replaceAll('-', ' ')))
      .slice(0, 1);
  };
