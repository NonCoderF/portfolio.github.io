...
import { callOpenAIChat, streamOpenAIChat, type OpenAIMessage } from "./openaiClient.ts";
...
import { buildFallbackUnderstanding, type QueryUnderstanding } from "./retrieval/queryUnderstanding.ts";
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    const requestId = req.headers.get("x-digital-nizam-client-request-id") || createPipelineRequestId();
    ...
    understanding = buildFallbackUnderstanding(history.length > 0 ? "personal" : localScope, detected.intent);
    ...
    const retrievalQuestion = [prompt, ...history.slice(-6).map((message) => message.content)].join("\n");
    const retrieval = await retrieveSemanticKnowledge(OPENAI_API_KEY, retrievalQuestion, understanding);
    addPerformanceStage(performanceTrace, "retrievalMs", retrievalStartedAt);
    performanceTrace.stages.embeddingMs = 0;
    performanceTrace.stages.keywordSearchMs = performanceTrace.stages.retrievalMs;
    ...
    const performancePath = "FAST";
    logPipelineDebug(requestId, "router", {
      performancePath,
      pathReason: "local hybrid retrieval; final model handles context and relevance",
    });
    const rerankingStartedAt = performance.now();
    const evidence = {
      chunks: retrieval.chunks,
      connections: [] as EvidenceConnection[],
      strategy: "retrieval_order" as const,
    };
    addPerformanceStage(performanceTrace, "rerankingMs", rerankingStartedAt);
    logPipelineDebug(requestId, "reranked_personal_evidence", {
      rerankerPath: "local_fast_path",
    ...
    // Stream both general and personal answers. Generation-time evidence rules protect personal claims;
    // blocking verification remains observable only on the exceptional/debug path.
    const wantsStream = req.headers.get("accept")?.includes("text/event-stream") === true;
    if (!includeDebugTrace && wantsStream) {
      const streamEvidenceResources = understanding.contextDependent && !understanding.shouldSurfaceResources
        ? { resources: [], diagnostics: [] }
        : selectEvidenceBackedResources(understanding.topics, chunks);
      const streamExplicitResources = understanding.shouldSurfaceResources
        ? resolveRelevantResources(prompt, understanding.topics, [...builtPrompt.memories, ...chunks])
        : [];
      const streamResources = [...streamEvidenceResources.resources, ...streamExplicitResources]
        .filter((resource, index, all) => all.findIndex((candidate) => candidate.type === resource.type && candidate.id === resource.id) === index)
        .slice(0, 1);
      let streamed: Response;
      try {
        streamed = await streamOpenAIChat(OPENAI_API_KEY, messages, { ...remoteMetrics, purpose: "stream_generation" });
      } catch (streamError) {
        console.error("Streaming generation request failed; falling back to buffered generation", streamError);
        streamed = new Response(null, { status: 599 });
      }
      if (streamed.ok && streamed.body) {
        ...
        let sseBuffer = "";
        let firstModelTokenAt: number | undefined;
        let firstClientWriteAt: number | undefined;
        const output = new ReadableStream<Uint8Array>({
          async pull(controller) {
            const { done, value } = await reader.read();
            if (done) {
              controller.enqueue(encoder.encode(`event: done\ndata: ${JSON.stringify({ resources: streamResources })}\n\n`));
              controller.close();
              logPerformanceTrace(requestId, performanceTrace, {
                performancePath,
                stream: true,
                firstModelTokenRelativeMs: firstModelTokenAt === undefined ? null : Math.round((firstModelTokenAt - performanceTrace.startedAt) * 100) / 100,
                firstClientWriteRelativeMs: firstClientWriteAt === undefined ? null : Math.round((firstClientWriteAt - performanceTrace.startedAt) * 100) / 100,
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (typeof delta === "string" && delta) {
                  const isFirstToken = firstModelTokenAt === undefined;
                  firstModelTokenAt ??= performance.now();
                  firstClientWriteAt ??= performance.now();
                  controller.enqueue(encoder.encode(`event: text\ndata: ${JSON.stringify(delta)}\n\n`));
                  if (isFirstToken) logPipelineDebug(requestId, "final_generation_context", {
                    streamEvent: "first_token_and_client_write",
                    firstModelTokenRelativeMs: Math.round((firstModelTokenAt - performanceTrace.startedAt) * 100) / 100,
                    firstClientWriteRelativeMs: Math.round((firstClientWriteAt - performanceTrace.startedAt) * 100) / 100,
                  });
                }
              } catch (_error) {
                // SSE chunks may split JSON records; the next chunk remains authoritative.
              }
            }
          },
          cancel() {
