(() => {
  ...
  const resourceHrefs = { 'adaptive-exercise-recognition':'#adaptive-exercise-recognition', archguard:'https://github.com/NonCoderF/ArchGuard', sonicbridge:'#sonicbridge', 'tapori-ai':'https://github.com/NonCoderF/tapori-ai/tree/master-flutter', 'biometric-sdk':'https://github.com/NonCoderF/biometric-sdk', 'sally-launcher':'https://play.google.com/store/apps/details?id=com.code.amiles.sally.free&hl=en_IN' };
  const appendResourceCards = resources => (Array.isArray(resources) ? resources : []).slice(0, 2).map(resource => { const href = resource.url || resourceHrefs[resource.id]; if (!href) return ''; return `<div class="ai-project-card"><strong>${escapeAI(resource.title || resource.id)}</strong><span class="ai-project-status">${escapeAI(String(resource.type || 'resource').toUpperCase())}</span>${resource.subtitle ? `<small>${escapeAI(resource.subtitle)}</small>` : ''}<div class="ai-project-actions"><a href="${escapeAI(href)}" ${String(href).startsWith('http') ? 'target="_blank" rel="noopener noreferrer"' : ''}><i class="bi bi-arrow-up-right"></i> Open resource</a></div></div>`; }).join('');
  const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
  const streamResponse = async (text, resources = []) => { openAssistant(); const panel = document.createElement('div'); panel.className = 'assistant-msg ai-response reveal'; panel.innerHTML = '<span class="ai-message-label">NIZAM / RESPONSE</span><div class="ai-stream-text"></div>'; assistantMessages?.appendChild(panel); scrollAssistantBottom('auto'); const stream = $('.ai-stream-text', panel); for (const character of String(text)) { stream.textContent += character; scrollAssistantBottom('auto'); await delay(character === '\n' ? 110 : /[.!?,:;]/.test(character) ? 55 : 9); } stream.innerHTML = formatAIResponse(text); const cards = appendResourceCards(resources); if (cards) panel.insertAdjacentHTML('beforeend', cards); scrollAssistantBottom(); return panel; };
